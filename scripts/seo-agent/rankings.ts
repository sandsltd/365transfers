...
function parseKeywordsFromStrategy(strategyContent: string): string[] {
  ...
  // 5-column table: | # | Keyword | Position | Last Checked | Target Page |
  const tableRowRegex =
...
export async function checkRankings(): Promise<RankingsResult> {
  ...
  for (const ranking of rankings) {
    ...
    // 5-column table update: | # | Keyword | Position | Last Checked | Target Page |
    const rowRegex = new RegExp(
      `(\\|\\s*\\d+\\s*\\|\\s*${escapeRegex(ranking.keyword)}\\s*\\|)\\s*[^|]+\\s*\\|\\s*[^|]+\\s*\\|(\\s*[^|]+\\s*\\|)`,
      "g"
    );
    updatedStrategy = updatedStrategy.replace(
      rowRegex,
      `$1 ${positionStr} | ${today} |$2`
    );
  }
